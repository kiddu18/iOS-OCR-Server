    ...
    def process(self, text_blocks, boxes, result):
        ...
        for box in candidate_boxes:
            ...
            if is_valid_cui(numbers_only):
                result.cui = numbers_only
                result.cuiRequiresVerification = False
                verify_with_anaf(numbers_only, result, self.simulate_timeout)
                result.cui = numbers_only
                return
...
class AccountingOrchestrator:
    ...
    def process_ocr_result(self, boxes, buyer_cui: str = None) -> AccountingResult:
        text_blocks = []
...
def run_tests():
    print("=" * 60)
    print("RUNNING SPATIAL OCR PARSING SIMULATOR TESTS (ALIGNED WITH SWIFT)")
    print("=" * 60)
    ...
    res1 = orchestrator.process_ocr_result(s1_boxes)
    ...
    res2_match = orchestrator.process_ocr_result(s2_boxes, buyer_cui="8609468")
    print(res2_match)
    ...
    res2_mismatch = orchestrator.process_ocr_result(s2_boxes, buyer_cui="2816464")
    print(res2_mismatch)
    ...
    res3 = orchestrator.process_ocr_result(s3_boxes)
    print(res3)
    ...
    res4a = orchestrator.process_ocr_result(s4a_boxes)
    print(res4a)
    ...
    res4b = orchestrator.process_ocr_result(s4b_boxes)
    print(res4b)
    ...
    # Sub-case A: Split Decimal Box (Swift-aligned: expected to fail extraction)
    ...
    print(res5a)
    assert res5a.totalAmount is None, f"Expected total to be None due to lack of box joining, got {res5a.totalAmount}"
    assert res5a.totalRequiresVerification is True, f"Expected totalRequiresVerification to be True, got {res5a.totalRequiresVerification}"
    ...
    res5b = orchestrator.process_ocr_result(s5b_boxes)
    print(res5b)
    ...
    res5c = orchestrator_timeout.process_ocr_result(s5c_boxes)
    print(res5c)
